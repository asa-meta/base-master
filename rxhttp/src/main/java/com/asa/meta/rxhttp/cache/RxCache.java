package com.asa.meta.rxhttp.cache;

import android.content.Context;
import android.os.Environment;
import android.os.StatFs;

import com.asa.meta.rxhttp.cache.converter.SerializableDiskConverter;
import com.asa.meta.rxhttp.cache.core.CacheCore;
import com.asa.meta.rxhttp.cache.core.LruDiskCache;
import com.asa.meta.rxhttp.cache.model.CacheMode;
import com.asa.meta.rxhttp.cache.model.CacheResult;
import com.asa.meta.rxhttp.cache.stategy.IStrategy;
...
public final class RxCache {
     ...
     */
    public <T> ObservableTransformer<String, CacheResult> transformer(CacheMode cacheMode) {
        final IStrategy strategy = loadStrategy(cacheMode);//获取缓存策略
        return new ObservableTransformer<String, CacheResult>() {
            @Override
            public ObservableSource<CacheResult> apply(@NonNull Observable<String> upstream) {
                HttpLog.i("cackeKey=" + RxCache.this.cacheKey);

                return strategy.execute(RxCache.this, RxCache.this.cacheKey, RxCache.this.cacheTime, upstream);
            }
        };
    }
     ...
     * @param key 缓存key
     */
    public Observable<String> load(final String key) {
        return load(key, -1);
    }
     ...
     * @param time 保存时间
     */
    public Observable<String> load(final String key, final long time) {
     ...
     * @param value 缓存Value
     */
    public <T> Observable<Boolean> save(final String key, final String value) {
