...
import com.asa.meta.rxhttp.interceptor.BaseDynamicInterceptor;
import com.asa.meta.rxhttp.interceptor.CacheInterceptor;
import com.asa.meta.rxhttp.interceptor.CacheInterceptorOffline;
import com.asa.meta.rxhttp.interceptor.HeadersInterceptor;
import com.asa.meta.rxhttp.interceptor.NoCacheInterceptor;
import com.asa.meta.rxhttp.interfaces.IDiskConverter;
import com.asa.meta.rxhttp.main.RxHttp;
import com.asa.meta.rxhttp.model.HttpHeaders;
import com.asa.meta.rxhttp.model.HttpParams;
import com.asa.meta.rxhttp.transformer.CacheResultTramsformer;
...
import static com.asa.meta.rxhttp.main.RxHttp.getRxCache;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    private OkHttpClient.Builder generateOkClient() {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null
            ...
            if (cookies.size() > 0) {
                RxHttp.getCookieJar().addCookies(cookies);
            }
    ...
    private Retrofit.Builder generateRetrofit() {
        if (converterFactories.isEmpty() && adapterFactories.isEmpty()) {
            return RxHttp.getRetrofitBuilder().baseUrl(baseUrl);
        } else {
            ...
            if (!converterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                List<Converter.Factory> listConverterFactory = RxHttp.getRetrofit().converterFactories();
            ...
            if (!adapterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                List<CallAdapter.Factory> listAdapterFactory = RxHttp.getRetrofit().callAdapterFactories();
...
//    public Observable executeRetry() {
//        return build().generateRequest().map(new StringBodyFunc())
//                .compose(isSyncRequest ? RxUtil.toMain() : RxUtil.ioToMain())
//                .compose(rxCache.transformer(cacheMode))
